...
public class SceneViewCameraProxy : MonoBehaviour
{
	...
	static void CheckMe (SceneView sv)
	{
		...
		SceneViewCameraProxy[] cameraFilters = Camera.main.GetComponents<SceneViewCameraProxy> ();
		SceneViewCameraProxy[] sceneFilters = sv.camera.GetComponents<SceneViewCameraProxy> ();
	...
	{
		SceneViewCameraProxy filter;
		while (filter = sv.camera.GetComponent<SceneViewCameraProxy> ())
			DestroyImmediate (filter);
		
		foreach (SceneViewCameraProxy f in Camera.main.GetComponents<SceneViewCameraProxy> ())
		{
			SceneViewCameraProxy newFilter = sv.camera.gameObject.AddComponent (f.GetType()) as SceneViewCameraProxy;
